...
export default class WindowsClipboard {
    async readText() {
        ...
        try {
            ...
            result = await execa(`powershell -Command Add-Type -AssemblyName System.Windows.Forms; "if([Windows.Forms.Clipboard]::ContainsText()) {$clip=[Windows.Forms.Clipboard]::GetText(); if ($clip -ne $null) {return $clip }} "`, {
                stripFinalNewline: true,
            });
        }
    ...
    async readImage(file) {
        ...
        try {
            ...
            console.log('A try with fallback with powershell');
            result = await execa(`powershell -Command Add-Type -AssemblyName System.Windows.Forms; "$clip=[Windows.Forms.Clipboard]::GetImage();if ($clip -ne $null) { $converter = New-Object -TypeName System.Drawing.ImageConverter;$byte_vec = $converter.ConvertTo($clip, [byte[]]); $EncodedText =[Convert]::ToBase64String($byte_vec); return $EncodedText }"`);
        }
        if (result && 'stderr' in result && result.stderr) {
    ...
    async readFiles() {
        ...
        try {
            ...
            console.log('A try with fallback with powershell');
            result = await execa(`powershell -Command Add-Type -AssemblyName System.Windows.Forms; "if ([Windows.Forms.Clipboard]::ContainsFileDropList()) {$files = [Windows.Forms.Clipboard]::GetFileDropList(); return $files"}`, {
                stripFinalNewline: true,
            });
        }
        if (result && 'stderr' in result && result.stderr) {
    ...
    async writeFiles(...files) {
        ...
        try {
            ...
            result = await execa(`powershell -Command Add-Type -AssemblyName System.Windows.Forms; "$files = [System.Collections.Specialized.StringCollection]::new(); $files.AddRange(@(${formattedFiles.join(',')})); [Windows.Forms.Clipboard]::SetFileDropList($files);`, {
                stripFinalNewline: false,
            });
        }
        if (result && 'stderr' in result && result.stderr) {
